...
- OpenSpec 路径请写全，优先使用 `openspec/changes/...` 和 `openspec/specs/...`，不要只写 `changes/...`
- Voice AI 的 `submit` 是“提交解析并判定”，不保证直接入库；如果返回 `needs_confirmation`，确认弹窗通常是预期行为
- Voice AI 服务端必须保留 fallback path：Workers AI 不可用时仍应能走 heuristic parser
- 词表扩充优先补分类名词和场景词，不要主要依赖宽泛动词或整句模板
- Web 语音识别只是文本输入增强层，手输和编辑能力必须始终可用
- 这次语音能力的复盘见 [retrospective.md](/Users/tongtong/AndroidStudioProjects/MoneyJar/openspec/changes/archive/2026-04-26-voice-ai-text-first-entry/retrospective.md)
- 通用开发提醒见 [docs/dev-tips.md](/Users/tongtong/AndroidStudioProjects/MoneyJar/docs/dev-tips.md)
